...
[HttpGet]public async Task<ActionResult> GetAll([FromQuery]int page=1,[FromQuery]int pageSize=20,[FromQuery]string? search=null,[FromQuery]int? tipo=null,[FromQuery]int? stato=null)=>Ok(ApiResponse<PagedResult<Contenzioso>>.Ok(await _s.GetAllAsync(page,pageSize,search,tipo,stato)));
[HttpGet("{id}")]public async Task<ActionResult> GetById(Guid id){var i=await _s.GetByIdAsync(id);return i==null?NotFound(ApiResponse.Fail("Non trovato")):Ok(ApiResponse<Contenzioso>.Ok(i));}
[HttpPost]public async Task<ActionResult> Create([FromBody]CreateContenziosoRequest r){var i=await _s.CreateAsync(r);return CreatedAtAction(nameof(GetById),new{id=i.Id},ApiResponse<Contenzioso>.Ok(i));}
[HttpPut("{id}")]public async Task<ActionResult> Update(Guid id,[FromBody]UpdateContenziosoRequest r){var i=await _s.UpdateAsync(id,r);return i==null?NotFound(ApiResponse.Fail("Non trovato")):Ok(ApiResponse<Contenzioso>.Ok(i));}
